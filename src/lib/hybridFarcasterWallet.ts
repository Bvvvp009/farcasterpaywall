// Hybrid Farcaster Wallet Solution
// Uses external RPC for contract interactions + Farcaster native actions for payments
...
// External RPC for contract interactions (since Farcaster wallet doesn't support eth_call, eth_estimateGas, etc.)
...
/**
 * Get hybrid wallet status - checks both Farcaster and external capabilities
 */
export async function getHybridWalletStatus(): Promise<HybridWalletStatus> {
  try {
    ...
    console.log('✅ Hybrid wallet status:', {
      ...
      canUseContractCalls: false // Farcaster wallet doesn't support contract calls
    })
...
/**
 * Register content using external RPC (since Farcaster wallet doesn't support contract calls)
 */
...
/**
 * Complete content access flow using hybrid approach
 */
export async function accessContentHybrid(
  contentId: string
): Promise<{ success: boolean; hasAccess: boolean; error?: string; decryptedContent?: string }> {
  try {
    console.log('🎭 Starting hybrid content access flow...')
    ...
    const hasAccess = await checkContentAccessWithExternalRPC(contentId, userAddress)
    ...
    const contentDetails = await getContentDetailsWithExternalRPC(contentId)
    if (!contentDetails) {
    ...
    console.error('❌ Error in hybrid content access:', error)
